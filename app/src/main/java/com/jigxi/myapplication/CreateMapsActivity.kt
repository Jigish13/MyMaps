...
class CreateMapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // providing user information on how to add marker
        // if mapFragment is not null, pass it as an object in SnackBar
    ...
    private fun showAlertDialog(latLng: LatLng) {
        ...
        // add marker only if user has clicked on OK button
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            val title = placeFormView.findViewById<EditText>(R.id.etTitle).text.toString()
            val description = placeFormView.findViewById<EditText>(R.id.etDescription).text.toString()
            if (title.trim().isEmpty() || description.trim().isEmpty()) {
                Toast.makeText(this, "Place must not have non empty title or description", Toast.LENGTH_LONG).show()
